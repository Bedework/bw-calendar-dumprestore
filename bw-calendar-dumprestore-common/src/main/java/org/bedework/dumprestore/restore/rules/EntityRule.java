...
public class EntityRule extends RestoreRule implements Defs {
  ...
  static {
    ...
    classes.put(objectCalendar, BwCollection.class.getName());
    classes.put(objectCollection, BwCollection.class.getName());
  ...
  public void begin(final String ns, final String name, final Attributes att) throws Exception {
    ...
    if (debug()) {
      debug("About to push entity for " + name);
    }

    final String className = classes.get(name);
   ...
   * @throws Exception
   */
  public void fixSharableEntity(final ShareableEntity entity,
                                final String name) throws Exception {
    try {
      final var dbent = (BwDbentity<?>)entity;
      if (entity.getCreatorHref() == null) {
        warn(name + " " + dbent.getHref() + " has no creator, set to " +
      ...
      if (entity.getOwnerHref() == null) {
        warn(name + " " + dbent.getHref() + " has no owner, set to " +
            globals.getPublicUser().getAccount());

        entity.setOwnerHref(globals.getPublicUser().getPrincipalRef());

        if (!entity.getPublick()) {
          warn(name + " " + dbent.getHref() + " is NOT public");
        }
      }
    } catch (final Throwable t) {
